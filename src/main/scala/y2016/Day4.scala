...
object Day4 extends App {
  ...
  val rooms = data.collect {
    case Thing(pattern, sectorId, checksum)
        if toChecksum(pattern).startsWith(checksum) =>
      (decrypt(pattern, sectorId.toInt), sectorId.toInt)
  }

  rooms.foreach(println)

  def toChecksum(s: String): String = {
    val x = s.foldLeft(Map.empty[Char, Int]) { case (acc, c) =>
      if (c != '-') {
    ...
    given Ordering[(Char, Int)] with
      override def compare(x: (Char, Int), y: (Char, Int)): Int =
        if x._2 == y._2 then x._1.compare(y._1)
