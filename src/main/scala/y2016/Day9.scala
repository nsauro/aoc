    ...
    def doTheThing(remaining: String, acc: String): String = {
      if (remaining.isEmpty) {
        ...
        if (openingIndex == -1) { // done
          ...
          val (preamble, expansion) =
            remaining.splitAt(openingIndex) // "abc", "(1x2)stuff"
          ...
          if (closingIndex == -1) { // not a expansion clause
            ...
            val (toTakeS, timesS) =
              clause.tail.splitAt(clause.tail.indexOf("x"))
      ...
      if (remaining.isEmpty) {
        ...
        if (openingIndex == -1) { // done
          ...
          if (closingIndex == -1) { // not a expansion clause
            ...
            val (toRepeat, last) = closing.tail.splitAt(toTake)
            val repeated =
              expand2(toRepeat) * times // Seq.fill(times)(expand2(toRepeat))
            // val block = Seq.fill(times)(toRepeat)
