...
object Day17 extends App {
  ...
  println(
    bfsLongest(Set.empty, Seq(Location(0, 0, Seq.empty)), (3, 3), Seq.empty)
  )
  ...
  def bfsLongest(
    ...
    if (queue.isEmpty) {
      ...
      bfsLongest(
        ...
        queue.tail,
        target,
        matches :+ queue.head.path.mkString("")
      )
    } else {
      val neighbors = getNeighbors(queue.head, visited, queue.tail)
      val nextQueue = queue.tail ++ neighbors
  ...
  def getNeighbors(
    ...
    val rawNeighbors = Seq(
      (
        hash(0),
        location.copy(y = location.y - 1, path = location.path :+ 'U')
      ), // up
      (
        hash(1),
        location.copy(y = location.y + 1, path = location.path :+ 'D')
      ), // up:
      (
        hash(2),
        location.copy(x = location.x - 1, path = location.path :+ 'L')
      ), // left:
      (
        hash(3),
        location.copy(x = location.x + 1, path = location.path :+ 'R')
      ) // left:
    )
