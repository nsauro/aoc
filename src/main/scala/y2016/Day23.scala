...
object Day23 extends App {
  ...
  def execute(registers: Map[String, Int], curInstr: Int): Map[String, Int] = {
    ...
    if (curInstr >= data.length) {
      ...
      data(curInstr) match {
        case CopyCell(fromRegister, toRegister) =>
          execute(
            registers + (toRegister -> registers(fromRegister)),
            curInstr + 1
          )
        case CopyValue(value, register) =>
          execute(registers + (register -> value.toInt), curInstr + 1)
        case Inc(register) =>
          execute(
        ...
        case JnzCellCell(register, direction) => {
          val next =
            if (registers(register) != 0) curInstr + registers(direction)
            else curInstr + 1
        ...
        case JnzCellInt(register, direction) => {
          val next =
            if (registers(register) != 0) curInstr + direction.toInt
            else curInstr + 1
          val realNext = if (next == curInstr) curInstr + 1 else next
          execute(registers, realNext)
        }
        case JnzIntCell(register, direction) => {
          val next =
            if (register.toInt != 0) curInstr + registers(direction)
            else curInstr + 1
          val realNext = if (next == curInstr) curInstr + 1 else next
          execute(registers, realNext)
        }
        case JnzIntInt(register, direction) => {
          val next =
            if (register.toInt != 0) curInstr + direction.toInt
            else curInstr + 1
          val realNext = if (next == curInstr) curInstr + 1 else next
          execute(registers, realNext)
        }
