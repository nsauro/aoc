...
object Day24 extends App{
  ...
  val res = (0 to maxPoint).permutations.filter(_.head == 0).map{ x =>
    x.sliding(2).map(y => points.getOrElse(s"${y.head}${y.last}", points(s"${y.last}${y.head}"))).sum
  }.min

  println(res)

  val res2 = (0 to maxPoint).permutations.filter(_.head == 0).map { x =>
    val all = x.toSeq :+ 0
    all.sliding(2).map(y => points.getOrElse(s"${y.head}${y.last}", points(s"${y.last}${y.head}"))).sum
  }.min
