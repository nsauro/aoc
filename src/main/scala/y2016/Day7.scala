...
object Day7 extends App {
  ...
  val data =
    Source.fromResource("2016/7.data").getLines().toSeq.map(_.toCharArray)
  ...
  def isSSL(s: Seq[Char]): Boolean = {
    ...
    def getSequences(
      ...
      if (remaining.isEmpty) {
        ...
        val (toCheck, nextCheck) = remaining.splitAt(boundLoc)
        val (updatedAba, updatedBab) =
          if (inBracket) (aba, bab ++ getAbas(toCheck))
          else (aba ++ getAbas(toCheck), bab)
        val next = if (nextCheck.isEmpty) nextCheck else nextCheck.tail
