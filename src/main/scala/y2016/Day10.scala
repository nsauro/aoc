...
object Day10 extends App {
  ...
  val GiveInst =
    raw"""bot (\d+) gives low to (.+) (\d+) and high to (.+) (\d+)""".r
  ...
  println(
    executeUntilDone(bots, Map.empty)
      .filter(x => Set(0, 1, 2).contains(x._1.id))
      .map(_._2.chips.head)
      .product
  )
    ...
    if (ins.isEmpty) {
      ...
      ins.head match {
        case InitValue(amt, botId) => {
          val updatedBot = bots
            .get(BotId(botId.toInt))
            .fold(Bot(BotId(botId.toInt), Seq(amt.toInt), BotId(0), BotId(0)))(
              x => x.copy(chips = x.chips :+ amt.toInt)
            )
          ...
          val updatedBot = bots
            ...
            .fold(
              Bot(
                BotId(botId.toInt),
                Seq.empty,
                Id(lowDest, lowAmt.toInt),
                Id(highDest, highAmt.toInt)
              )
            )(x =>
              x.copy(
                lowTo = Id(lowDest, lowAmt.toInt),
                highTo = Id(highDest, highAmt.toInt)
              )
            )
          parseInstructions(ins.tail, bots + (updatedBot.id -> updatedBot))
        }
      ...
      case None => {
        ...
        nextBot match {
          ...
          case Some(b) => {
            ...
            val (lowUpdatedOutputs, lowUpdatedBots) =
              update(b.lowTo, outputs, bots, b.chips.min)
            val (highUpdatedOutputs, highUpdatedBots) =
              update(b.highTo, lowUpdatedOutputs, lowUpdatedBots, b.chips.max)
  ...
  def update(
      ...
      bots: Map[BotId, Bot],
      chip: Int
  ): (Map[OutputId, Output], Map[BotId, Bot]) = {
    id match {
  ...
  def updateOutputs(
    ...
    outputs.get(outputId) match {
      ...
      case Some(x) =>
        outputs + (outputId -> x.addChip(chip).asInstanceOf[Output])
    }
  }
  ...
  case class Bot(id: BotId, chips: Seq[Int], lowTo: Id, highTo: Id)
      extends Container[BotId] {
    override def addChip(c: Int): Container[BotId] =
      this.copy(chips = chips :+ c)
  }

  case class Output(id: OutputId, chips: Seq[Int]) extends Container[OutputId] {
    override def addChip(c: Int): Container[OutputId] =
      this.copy(chips = chips :+ c)
  }
