...
object Day05 extends App{
  ...
  def findMin(start: Long, range:Long, cur: Long, curMin : Long) : Long = {
    ...
    if(cur == range){
      ...
      val loc = ranges.foldLeft(Seq(start + cur)){case (acc, rm) =>
  ...
  def parseInput(i : Seq[String], acc: Seq[RangeMap], curr: Option[RangeMap]) : Seq[RangeMap] = {

    if(i.isEmpty){
      acc ++ curr
    }else{
      i.head match{
        case RangeHeader(s, d) => parseInput(i.tail, acc, Some(RangeMap(s, d, Seq.empty)))
        case RangeData(d, s, r) => parseInput(i.tail, acc, curr.map(_.addRange(Range(d.toLong, s.toLong, r.toLong))))
