...
object Day22 extends App {
  ...
  // sorted by lowest z
  val shapes: Seq[(Shape, Int)] = data.zipWithIndex
    .map {
  ...
  // condense
  val initialLayers =
    shapes.foldLeft(mutable.HashMap.empty[Int, ListBuffer[Shape]]) {
      case (layers, (shape, level)) =>
  ...
  val res = shapes.map { case (shape, _) =>
    ...
    val layers =
      removed.foldLeft(mutable.HashMap.empty[Int, ListBuffer[Shape]]) {
        case (layers, (shape, level)) =>
          // println(s"finding home for $shape -- $level")
          val (updated, _) = condenseShape(shape, level, layers)
          updated
  ...
  def condenseShape(
      shape: Shape,
      level: Int,
      layers: mutable.HashMap[Int, ListBuffer[Shape]]
  ): (mutable.HashMap[Int, ListBuffer[Shape]], Int) = {
  ...
  def countDisintegrateableBricks(
      layers: mutable.HashMap[Int, ListBuffer[Shape]]
  ): Int = {
      ...
      if (layer == layers.keys.max) { // top most layer can all disintegrate
        ...
        layers(layer).filterNot(_.isVerticalStub).count { shape =>
          ...
          if (layerAbove > layers.keys.max) {
            ...
            val overlaps = layerToCheck.filter(
              _.overlaps(shape)
            ) // all shapes which overlap current
            val layerWithoutShape =
              layers(layerAbove - 1).filterNot(_.sameWithoutStub(shape))
            // check overlaps again..make sure
            val stillOverlaps =
              overlaps.count(x => layerWithoutShape.exists(_.overlaps(x)))
