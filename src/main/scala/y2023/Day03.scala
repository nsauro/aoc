...
object Day03 extends App {
  ...
  def getNumbers(
      s: Seq[(Char, Int)],
      cur: (String, Int, Int) = ("", -1, -1),
      acc: Seq[(Int, Int, Int)] = Seq.empty
  ): Seq[(Int, Int, Int)] = {
    ...
    if (s.isEmpty) { // nothing left..if we are processing a number, finish it up
      ...
      if (s.head._1.isDigit && cur._1.isEmpty) {
        ...
        getNumbers(
          s.tail,
          ("", -1, -1),
          acc :+ (cur._1.toInt, cur._2, s.head._2 - 1)
        )
  ...
  def isAdjacent(rowNum: Int, start: Int, end: Int) = {
    ...
    val preceding =
      if (start == 0) Seq.empty[Char] else Seq(data(rowNum)(start - 1))
    // get succeeding characters
    val succeeding =
      if (end == data.head.length - 1) Seq.empty[Char]
      else Seq(data(rowNum)(end + 1))
    ...
    val above =
      if (rowNum == 0) Seq.empty[Char]
      else data(rowNum - 1).substring(min, max + 1).toSeq
    // get row below characters
    val below =
      if (rowNum == data.length - 1) Seq.empty[Char]
      else (data(rowNum + 1)).substring(min, max + 1).toSeq
  ...
  val gearRatio = data.zipWithIndex.map { case (x, i) =>
    x.zipWithIndex.collect { case ('*', gi) =>
      getGearRatio(i, gi)
    }.sum
  }.sum
  ...
  def getGearRatio(row: Int, index: Int): Long = {
    ...
    val above =
      if (row == 0) Seq.empty
        ...
        numbers(row - 1).filter { n =>
          index >= n._2 - 1 && index <= n._3 + 1
        }
    // numbers below
    val below =
      if (row == data.length - 1) Seq.empty
      else
        numbers(row + 1).filter { n =>
          index >= n._2 - 1 && index <= n._3 + 1
        }
