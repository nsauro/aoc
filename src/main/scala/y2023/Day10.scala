...
object Day10 extends App{
  ...
  def walk(cur: (Int, Int), from: (Int, Int), acc: Seq[(Int, Int)]) : Seq[(Int, Int)] = {
    data(cur._1)(cur._2) match {
      case '|' =>
        val next = getNext(Seq(cur.copy(_1 = cur._1 + 1), cur.copy(_1 = cur._1 - 1)), from)
        walk(next, cur, acc :+ cur)
      case '-' =>
        val next = getNext(Seq(cur.copy(_2 = cur._2 + 1), cur.copy(_2 = cur._2 - 1)), from)
        walk(next, cur, acc :+ cur)
      case 'L' =>
        val next = getNext(Seq(cur.copy(_1 = cur._1 - 1), cur.copy(_2 = cur._2 + 1)), from)
        walk(next, cur, acc :+ cur)
      case 'J' =>
        val next = getNext(Seq(cur.copy(_1 = cur._1 - 1), cur.copy(_2 = cur._2 - 1)), from)
        walk(next, cur, acc :+ cur)
...
//-------- part 2 stuff
  ...
  //2x grid
  val newGrid = Array.fill(data.length * 2 + 2)(Array.fill(data.head.length * 2 + 2)('*'))
      ...
      if(r % 2 == 1){ //dilation row
        if(c % 2 == 1){ //dilation column
          ...
          if(res.contains(originalCoords)){
            newGrid(r)(c) = data(originalCoords._1)(originalCoords._2) //copy to new location
  ...
  def getNeighbors(point: (Int, Int), visited: Set[(Int, Int)]): Set[(Int, Int)] = {
    val neighbors = Set(
      (point._1 + 1, point._2),
      (point._1 - 1, point._2),
      (point._1, point._2 + 1),
