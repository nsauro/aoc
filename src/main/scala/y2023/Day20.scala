...
object Day20 extends App {
  ...
  val rawParams = data.foldLeft(Map.empty[String, Seq[String]]) {
    case (acc, str) =>
      val split = str.split("->")
  ...
  val cycles = Seq("ns", "bh", "dl", "vd").map { src =>
    ...
    val res = findHighSignalToZh(
      mutable.Queue(Signal(Low, "button", "broadcaster")),
      1,
      src,
      Seq.empty
    )
  ...
  def findCycle(
    ...
    if (queue.isEmpty) {
      if (modules.values.forall(_.inOriginalState) || presses == 1000) {
        ...
        findCycle(
          queue.addOne(Signal(Low, "button", "broadcaster")),
          presses + 1,
          lows,
          highs
        )
      ...
      val updatedQ = queue.foldLeft(newQ) { case (acc, sig) =>
        // println(sig)
        acc.addAll(
          modules.get(sig.des).map(_.execute(sig)).getOrElse(Seq.empty)
        )
      }
  ...
  def findHighSignalToZh(
      queue: mutable.Queue[Signal],
      presses: Int,
      src: String,
      signalsToZh: Seq[Signal]
  ): Int = {
    if (queue.isEmpty) {
      if (signalsToZh.nonEmpty && signalsToZh.head.value == High) {
        ...
        println(
          s"presses: $presses -- ${signalsToZh.size} -- ${signalsToZh.exists(_.value == High)}"
        )
        findHighSignalToZh(
          queue.addOne(Signal(Low, "button", "broadcaster")),
          presses + 1,
      ...
      val newQ = mutable.Queue.empty[Signal]
      val (updatedQ, updatedSignals) = queue.foldLeft((newQ, signalsToZh)) {
        case ((acc, zhAcc), sig) =>
          val updatedZhAcc =
            if (sig.src == src && sig.des == "zh") zhAcc :+ sig else zhAcc
          (
            acc.addAll(
              modules.get(sig.des).map(_.execute(sig)).getOrElse(Seq.empty)
            ),
  ...
  case class Signal(value: Int, src: String, des: String) {
    override val toString: String = s"$src -${
        if (value == High) { "high" }
        else { "low" }
      }-> $des"
  }
