...
object Day19 extends App {
  ...
  val shapes = shapeParameters.tail.map { case ShapeParameters(x, m, a, s) =>
    Shape(x.toInt, m.toInt, a.toInt, s.toInt)
  }
  ...
  object Step {
    ...
    def aLess(a: Int, res: Result) = new Step {
      override def exec(shape: Shape): Option[Result] =
        Option.when(shape.a < a)(res)
    }
  ...
  val allRanges =
    findRanges("in", Range(1, 4000, 1, 4000, 1, 4000, 1, 4000)).distinct
  ...
  def findRanges(name: String, range: Range): Seq[Range] = {
    ...
    val (_, acc) = rawFlows(name).split(",").foldLeft(range, Seq.empty[Range]) {
      case ((temp, acc), rawString) =>
        rawString match {
          case XGreater(amt, "R") => {
          ...
          case XGreater(amt, "A") => {
            (
              temp.copy(xEnd = amt.toInt),
              acc :+ (temp.copy(xStart = amt.toInt + 1))
            )
          }
          case XGreater(amt, des) => {
            (
              temp.copy(xEnd = amt.toInt),
              acc ++ findRanges(des, temp.copy(xStart = amt.toInt + 1))
            )
          }
          ...
          case XLess(amt, "A") => {
            (
              temp.copy(xStart = amt.toInt),
              acc :+ (temp.copy(xEnd = amt.toInt - 1))
            )
          }
          case XLess(amt, des) => {
            (
              temp.copy(xStart = amt.toInt),
              acc ++ findRanges(des, temp.copy(xEnd = amt.toInt - 1))
            )
          }
          ...
          case MGreater(amt, "A") => {
            (
              temp.copy(mEnd = amt.toInt),
              acc :+ (temp.copy(mStart = amt.toInt + 1))
            )
          }
          case MGreater(amt, des) => {
            (
              temp.copy(mEnd = amt.toInt),
              acc ++ findRanges(des, temp.copy(mStart = amt.toInt + 1))
            )
          }
          ...
          case MLess(amt, "A") => {
            (
              temp.copy(mStart = amt.toInt),
              acc :+ (temp.copy(mEnd = amt.toInt - 1))
            )
          }
          case MLess(amt, des) => {
            (
              temp.copy(mStart = amt.toInt),
              acc ++ findRanges(des, temp.copy(mEnd = amt.toInt - 1))
            )
          }
          ...
          case AGreater(amt, "A") => {
            (
              temp.copy(aEnd = amt.toInt),
              acc :+ temp.copy(aStart = amt.toInt + 1)
            )
          }
          case AGreater(amt, des) => {
            (
              temp.copy(aEnd = amt.toInt),
              acc ++ findRanges(des, temp.copy(aStart = amt.toInt + 1))
            )
          }
          ...
          case ALess(amt, "A") => {
            (
              temp.copy(aStart = amt.toInt),
              acc :+ temp.copy(aEnd = amt.toInt - 1)
            )
          }
          case ALess(amt, des) => {
            (
              temp.copy(aStart = amt.toInt),
              acc ++ findRanges(des, temp.copy(aEnd = amt.toInt - 1))
            )
          }
          ...
          case SGreater(amt, "A") => {
            (
              temp.copy(sEnd = amt.toInt),
              acc :+ temp.copy(sStart = amt.toInt + 1)
            )
          }
          case SGreater(amt, des) => {
            (
              temp.copy(sEnd = amt.toInt),
              acc ++ findRanges(des, temp.copy(sStart = amt.toInt + 1))
            )
          }
          ...
          case SLess(amt, "A") => {
            (
              temp.copy(sStart = amt.toInt),
              acc :+ (temp.copy(sEnd = amt.toInt - 1))
            )
          }
          case SLess(amt, des) => {
            (
              temp.copy(sStart = amt.toInt),
              acc ++ findRanges(des, temp.copy(sEnd = amt.toInt - 1))
            )
          }
