...
object Day24 extends App {
  ...
  val HailStoneParams =
    raw"""(-?\d+),\s*(-?\d+),\s*(-?\d+)\s*@\s*(-?\d+),\s*(-?\d+),\s*(-?\d+)""".r

  val data = Source.fromResource("2023/24.data").getLines().toSeq.map {
    case HailStoneParams(x, y, z, xd, yd, zd) =>
      HailStone(
        x.toDouble,
        y.toDouble,
        z.toDouble,
  ...
  case class HailStone(
    ...
    def intersection(h: HailStone): Option[(Double, Double)] = {
      ...
      Option.when(det.abs > 1.0e-6) {
        val detAt =
          (x * h.deltaY) + (-h.x * h.deltaY) + (-y * h.deltaX) + (h.y * h.deltaX)
        val detAs =
          (-y * deltaX) + (h.y * deltaX) + (x * deltaY) + (-h.x * deltaY)
        val t = detAt / det
        val s = detAs / det
        Option.when(t > 0 && s > 0) {
