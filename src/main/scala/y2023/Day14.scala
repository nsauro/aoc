...
object Day14 extends App{
  ...
  val raw = Source.fromResource("2023/14.data").getLines().toArray.map(_.toCharArray)
  ...
  def spin(cur: Array[Array[Char]], times: Int, max: Int, acc : Map[String, Int]) : Array[Array[Char]] = {
    if(times != max) {
      spin(cur)
  ...
  def findRestingSpot(grid: Array[Array[Char]], previousRow: Int, previousCol: Int,  nextRow : Int => Int, nextCol: Int => Int) : (Int, Int) = {
    val nextR = nextRow(previousRow)
    val nextC = nextCol(previousCol)
    if(nextR < 0 || nextC  < 0 || nextR == grid.length || nextC == grid.head.length){
