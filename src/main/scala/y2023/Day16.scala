...
object Day16 extends App {
  ...
  val raw =
    Source.fromResource("2023/16.data").getLines().toArray.map(_.toArray)

  val occupiedTiles =
    findPath(Tile(0, 0, Direction.East), mutable.HashSet.empty)
  ...
  def findPath(
      current: Tile,
      acc: mutable.HashSet[Tile]
  ): mutable.HashSet[Tile] = {
    if (
      current.row < 0 ||
      current.row == raw.length ||
      current.col < 0 ||
      current.col == raw.head.length ||
      acc.contains(current)
    ) { // at a bounds or we hit a loop
      acc
    } else {
    ...
    def rotateClockwise: Tile = {
      direction match {
        case Direction.North =>
          this.copy(col = col + 1, direction = Direction.East)
        case Direction.South =>
          this.copy(col = col - 1, direction = Direction.West)
        case Direction.East =>
          this.copy(row = row + 1, direction = Direction.South)
        case Direction.West =>
          this.copy(row = row - 1, direction = Direction.North)
      }
    }
