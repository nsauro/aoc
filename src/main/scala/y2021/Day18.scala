...
object Day18 extends App{
  ...
  def largestMagnitude = {
    val data2 = Source.fromResource("2021/18.data").getLines().toSeq.map(toBuffer)

    data2.combinations(2).map{
      case first :: second :: tail => {

        val firstSecondAddition = Magnitude.calculate(add(ListBuffer.from(first), ListBuffer.from(second)))
        val secondFirstAddition = Magnitude.calculate(add(ListBuffer.from(second), ListBuffer.from(first)))
...
object Splitter{
  ...
  def doSplit(l : ListBuffer[String]) : Boolean = {
    ...
    if(numberToSplit == -1){
      ...
      l(numberToSplit) = "["
      l.insertAll(numberToSplit + 1, Seq(left.toString, ",", right.toString, "]"))
      true
    }
  }
...
object Exploder {
  ...
  def addToRight(index : Int, number : Int, l : ListBuffer[String]) : Unit = {
    val rightIndex = l.zipWithIndex.find{case (v, i) =>
      i > index && isInt(v)
    }.map(_._2)
