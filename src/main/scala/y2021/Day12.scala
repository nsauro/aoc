...
object Day12 extends App {
 ...
 val data3 = Seq(
  ...
  val totalPaths = doTheThing2(
    Set.empty,
    SimpleTraversal(caves = Seq(Start), visitedLittleCaves = Set.empty)
  )
  ...
  def doTheThing(
    ...
    if (currentCave == End) {
      ...
      acc + nextCaves.toSeq
        .map(x => doTheThing(acc, x, updatedVisitedCaves, updatedPath))
        .sum
    }
  }
  ...
  def doTheThing2(acc: Set[Seq[Cave]], traversal: Traversal): Set[Seq[Cave]] = {
    ...
    if (traversal.lastVisitedCave == End) {
      ...
      val adjacentCaves = allPaths
        .collect {
          case path if path.contains(traversal.lastVisitedCave) =>
            path.getDestination(traversal.lastVisitedCave)
        }
      ...
      traversal match {
        case a: SimpleTraversal => {
          val (little, rest) =
            adjacentCaves.partition(_.isInstanceOf[LittleCave])

          val complexTraversals = little
            .map(_.asInstanceOf[LittleCave])
            .diff(a.visitedLittleCaves)
            .flatMap(x =>
              Seq(
                FilledComplexTraversal(
                  a.caves :+ x,
                  a.visitedLittleCaves
                ), // bb
          ...
          acc ++ ((complexTraversals ++ visitableNonLittle).toSeq.flatMap(x =>
            doTheThing2(acc, x)
          ))
...
case class SimpleTraversal(
    caves: Seq[Cave],
    visitedLittleCaves: Set[LittleCave]
) extends Traversal

case class FilledComplexTraversal(
    caves: Seq[Cave],
    visitedLittleCaves: Set[LittleCave]
) extends Traversal {
    ...
    cave match {
      ...
      case lc: LittleCave =>
        Some(
          this.copy(
            caves = caves :+ lc,
            visitedLittleCaves = visitedLittleCaves + lc
          )
        )
