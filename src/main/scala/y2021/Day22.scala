...
object Day22 extends App {
  ...
  val actions = data.map {
    case Action.Off(xMin, xMax, yMin, yMax, zMin, zMax) => Action.TurnOff(xMin.toInt, xMax.toInt, yMin.toInt, yMax.toInt, zMin.toInt, zMax.toInt)
    case Action.On(xMin, xMax, yMin, yMax, zMin, zMax) => Action.TurnOn(xMin.toInt, xMax.toInt, yMin.toInt, yMax.toInt, zMin.toInt, zMax.toInt)
  }

  val turnedOn = actions.foldLeft(Set.empty[(Int, Int, Int)]) { case (acc, action) =>
  ...
  sealed trait Action {
    ...
    private def withinBounds(axisMin: Int, axisMax: Int, min: Int, max: Int): Boolean = {
  ...
  object Action {
    ...
    val Off = raw"""off x=([\-0-9]+)\.\.([\-0-9]+),y=([\-0-9]+)\.\.([\-0-9]+),z=([\-0-9]+)\.\.([\-0-9]+)""".r
    val On = raw"""on x=([\-0-9]+)\.\.([\-0-9]+),y=([\-0-9]+)\.\.([\-0-9]+),z=([\-0-9]+)\.\.([\-0-9]+)""".r


    case class TurnOn(xMin: Int, xMax: Int, yMin: Int, yMax: Int, zMin: Int, zMax: Int) extends Action

    case class TurnOff(xMin: Int, xMax: Int, yMin: Int, yMax: Int, zMin: Int, zMax: Int) extends Action
