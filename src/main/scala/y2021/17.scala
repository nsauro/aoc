...
object Day17 extends App{
  ...
  def computeApex(x : Int, y : Int, xVelocity : Int, yVelocity : Int, highestY : Int) : Int = {
    //println(s"$x - $y")
    if(isInTarget(x, y)){
      highestY
      ...
      computeApex(newX, newY, decreaseX(xVelocity), decreaseY(yVelocity), newHighestY)
    }
  }

  def landsIt(x : Int, y : Int, xVelocity : Int, yVelocity : Int, highestY : Int) : Boolean = {
    //println(s"$x - $y")
    if(isInTarget(x, y)){
      true
    }else if(isPastTarget(x, y)){ //bad coords
      ...
      val newY = y + yVelocity
      val newHighestY = Math.max(newY, highestY)
      landsIt(newX, newY, decreaseX(xVelocity), decreaseY(yVelocity), newHighestY)
    }
  }
