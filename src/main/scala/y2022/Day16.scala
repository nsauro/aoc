...
object Day16 extends App {
  ...
  val rooms = input.map { x =>
    x.replace("valve ", "valves ")
      .replace("tunnel ", "tunnels ")
      .replace("lead ", "leads ") match {
      case s"Valve $id has flow rate=$rate; tunnels leads to valves $rooms" =>
        Room(id, rate.toInt, rooms.split(",").map(_.trim))
    }
  }
  ...
  val distances: Map[Room, Set[DistanceToRoom]] = computeDistancesToRoom(
    roomMapWithValves.values.toSet + aa
  )

  def computeDistancesToRoom(
      roomsWithValues: Set[Room]
  ): Map[Room, Set[DistanceToRoom]] = {
  ...
  def distanceTo(source: Room, destination: Room): Int = {
    ...
    def go(r: Room, visited: Set[Room], distance: Int): Option[Int] = {
      if (r == destination) {
        ...
        val nextRooms =
          r.leadsTo.map(x => roomMap(x)).filterNot(visited.contains)
        if (nextRooms.isEmpty) {
          None
        } else {
          nextRooms
            .flatMap(n => go(n, visited + r, distance + 1))
            .sorted
            .headOption
        }
      }
  ...
  def compute2(
    ...
    if (turnedOnValves.size == roomMapWithValves.size) {
      ...
      val roomDistances =
        distances(room).filterNot(x => turnedOnValves.contains(x.room))

      val dis =
        roomDistances.filterNot(x => x.distance + 1 > timeRemaining).map { rd =>
          compute2(
            rd.room,
            turnedOnValves + rd.room,
            timeRemaining - (rd.distance + 1),
            pressure + rd.room.flowRate * (timeRemaining - (rd.distance + 1))
          )
        }
      if (dis.isEmpty) {
        pressure
  ...
  def compute3(
    ...
    if (turnedOnValves.size == toVisit.size) {
      ...
      val roomDistances = distances(room)
        .filter(x => toVisit.contains(x.room))
        .filterNot(x => turnedOnValves.contains(x.room))
