...
object Day09 extends App{
  ...
  val visited = compute(data.tail, first, (10000,10000), (10000,10000), Set((10000,10000)))
  println(visited.size)


  val largerVisited = compute2(data.tail, first, Seq.fill(10)((10000, 10000)), Set((10000, 10000)))
  ...
  def compute(movements : Seq[String], curMove: Movement, h: (Int, Int), t: (Int, Int), tailVisited: Set[(Int, Int)]) : Set[(Int, Int)] = {
    ...
    if(movements.isEmpty && curMove.steps == 0){
      ...
      compute(movements, curMove.copy(steps = curMove.steps - 1), newH, newT, tailVisited + newT)
    }
  }


  def compute2(movements: Seq[String], curMove: Movement, knots: Seq[(Int,Int)], tailVisited: Set[(Int, Int)]): Set[(Int, Int)] = {
    ...
    if (movements.isEmpty && curMove.steps == 0) {
      ...
      compute2(movements, curMove.copy(steps = curMove.steps - 1), newKnots, tailVisited + newKnots.last)
    }
  }
