...
object Day17 extends App {
  ...
  val chamber = ListBuffer
    .fill(1)(ListBuffer.fill(7)('#'))
    .addAll(ListBuffer.fill(4)(ListBuffer.fill(7)('.')))
  ...
  def dropUntilCycleDetected(
      curPiece: Int,
      localChamber: ListBuffer[ListBuffer[Char]],
      highestBlock: Int
  ) = {
  ...
  def movePiece2(piece: Shape, currentPiece: Int, currentHighest: Int): Int = {
    ...
    if (down == moved) { // couldn't move
      ...
      chamber.addAll(
        ListBuffer.fill(rowsToAdd)(ListBuffer.fill(7)('.'))
      ) // add more rows
      // printChamber()
      highestBlock
    } else {
  ...
  def dropPieces(currentPiece: Int, maxPiece: Int, highestBlock: Int): Long = {
    if (currentPiece > maxPiece) {
      ...
      dropPieces(
        currentPiece + 1,
        maxPiece,
        Math.max(blockHighest, highestBlock)
      )
  ...
  def checkForCycle(piece: Shape, curHighest: Int, curPieceCount: Int) = {
    ...
    if (previousStarts.size < 3) {
      ...
      if (first == second) {
        ...
        println(
          s"cycle found!! ${piece.name} pieces: $piecesSeen -- height gained: $heightGained  $previousStarts"
        )
      }
      false
    }
    ...
    val (name, fillChar, points) = piece % 5 match {
      case 1 =>
        (
          "DASH",
          'D',
          (2 to 5).map(col => (highestBlock + 4, col))
        ) // -=-- piece
      case 2 =>
        (
          "PLUS",
      ...
      case 4 =>
        (
          "BAR",
          'B',
          (highestBlock + 4 to highestBlock + 7).map(x => (x, 2))
        ) // | piece
      case 0 =>
        (
          "BLOCK",
