...
object Day12 extends App {
  ...
  val data: Array[Array[Char]] =
    Source.fromResource("2022/Day12").getLines().toArray.map(_.toCharArray)
  ...
  def compute2(row: Int, column: Int, visitedNodes: Seq[(Int, Int)]): Int = {
    if (data(row)(column) == 'E') {
      ...
      val neighbors = Seq(
        ...
        (getValue(data(c._1)(c._2)) - getValue(
          data(row)(column)
        ) == 1 || getValue(data(c._1)(c._2)) - getValue(data(row)(column)) == 0)
      }
      if (neighbors.isEmpty) {
        cache.put((row -> column), -1)
        -1
      } else {
        val updated = neighbors
          .map { x =>
            cache.getOrElse(
              x,
              compute2(x._1, x._2, visitedNodes :+ (row -> column))
            )
          }
          .filter(_ != -1)
          .map(_ + 1)
