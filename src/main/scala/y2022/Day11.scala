...
object Day11 extends App{
  ...
  val myData = MMap(
    ...
    3 -> Monkey(3, ListBuffer(86, 61, 67, 88, 94, 69, 56, 91), _ + 7, 11, 6, 7, 0),
  ...
  val business = updatedMonkeys.sortBy(x => -x.itemsHandled).take(2).map(_.itemsHandled).reduce(_ * _)
  println(business)




  def compute(curRound : Int, maxRound : Int, monkeys : MMap[Int, Monkey]) : Seq[Monkey] = {
  ...
  //1183848012
  //14068749900
  case class Monkey(id : Int, items : ListBuffer[Long], worryUpdate : Long => Long, test :Int, trueMonkey: Int, falseMonkey:Int, var itemsHandled: Long)
}
