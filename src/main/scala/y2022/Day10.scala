...
object Day10 extends App {
  ...
  def compute(
      instr: Seq[String],
      curCycle: Int,
      curInstr: Option[AddInstr],
      x: Int,
      signalStrength: Int,
      interestedCycles: Set[Int]
  ): Int = {

    val (updatedStrength, updatedInterestedCycles) =
      if (interestedCycles.contains(curCycle)) {
    ...
    if (instr.isEmpty && curInstr.isEmpty) {
      ...
      curInstr match {
        case Some(v) =>
          val updatedRow =
            curRow :+ (if (
                         x == curCycle || x - 1 == curCycle || x + 1 == curCycle
                       ) '#'
                       else '.')
          val (nextRow, nextScreen) =
            if (updatedRow.size == 40) (Seq.empty, screen :+ updatedRow)
            else (updatedRow, screen)
        ...
        case None => {
          ...
          instr.head match {
            ...
            case s"addx $v" =>
              compute2(instr.tail, Some(v.toInt), x, nextRow, nextScreen)
          }
        }
      }
    }
