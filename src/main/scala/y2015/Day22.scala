...
object Day22 extends App {
  ...
  case object Drain extends Spell {
    ...
    override def cast(
        player: Wizard,
        boss: Boss
    ): (Wizard, Boss, Option[SpellEffect]) = {
      (
        player.copy(
          hitPointsRemaining = player.hitPointsRemaining + 2,
          manaRemaining = player.manaRemaining - cost
        ),
        boss.copy(hitPointsRemaining = boss.hitPointsRemaining - 2),
        None
      )
    }

    override def isCastable(
        currentMana: Int,
        activeEffects: Seq[SpellEffect]
    ): Boolean = currentMana >= cost
  }
  case object MagicMissile extends Spell {
    ...
    override def cast(
      ...
      (
        player.copy(manaRemaining = player.manaRemaining - cost),
        boss.copy(hitPointsRemaining = boss.hitPointsRemaining - 4),
        None
      )
    }
  ...
  case object NopSpell extends Spell {
    ...
    override def cast(
        player: Wizard,
        boss: Boss
    ): (Wizard, Boss, Option[SpellEffect]) = (player, boss, None)
  ...
  case class PoisonEffect(charges: Int) extends SpellEffect {
    override def cast(
      ...
      (
        player,
        boss.copy(hitPointsRemaining = boss.hitPointsRemaining - 3),
        effect
      )
    }
  }
  ...
  allSpells
    .map(sp => fight(wizard, boss, sp, Seq.empty, 0, WizardTurn, true))
    .min
  println(bestScore)
  ...
  def fight(
    ...
    if (w2.hitPointsRemaining <= 0) {
      ...
      // run effects
      val (updatedWizard, updatedBoss, remainingEffects) =
        activeEffects.foldLeft((w2, boss, Seq.empty[SpellEffect])) {
          case ((w, b, acc), e) => {
      ...
      if (updatedBoss.hitPointsRemaining <= 0) {
        ...
        turn match {
          ...
          case BossTurn => {
            val dmg =
              Math.max(updatedBoss.damageScore - updatedWizard.armorScore, 1)
            val ew = updatedWizard.copy(hitPointsRemaining =
              updatedWizard.hitPointsRemaining - dmg
            )
            if (ew.hitPointsRemaining <= 0) { // player is dead
              println("wiz is dead")
            } else {
              // both boss and wizard survived, run the next round with all affordable spells
              val affordableSpells = allSpells.filter(
                _.isCastable(ew.manaRemaining, remainingEffects)
              )
              (affordableSpells, remainingEffects) match {
                ...
                case (Nil, ef) =>
                  fight(
                    ...
                    WizardTurn,
                    hardMode
                  ) // no spells..but active effects..maybe he can still win
                case (spells, ef) => {
                  spells.foreach { s =>
                    fight(
                      ew,
