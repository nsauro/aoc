  ...
  def compute(regA : Int, regB: Int, instPointer : Int) : (Int, Int) = {
    ...
    if(instPointer >= data.length){
      ...
      data(instPointer) match {
        ...
        case s"jie a, $amt" => compute(regA, regB, computeOffset(instPointer, amt.toInt, regA % 2 == 0))
        case s"jie b, $amt" => compute(regA, regB, computeOffset(instPointer, amt.toInt, regB % 2 == 0))
        case s"jio a, $amt" => compute(regA, regB, computeOffset(instPointer, amt.toInt, regA == 1))
        case s"jio b, $amt" => compute(regA, regB, computeOffset(instPointer, amt.toInt, regB == 1))
      }
    }
  }
