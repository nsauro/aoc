...
object Day5 extends App {
  ...
  def isNice2(str: String): Boolean = {
    ...
    def hasNonOverlappingPairs(
        pairs: Seq[Seq[(Char, Int)]],
        charIndices: Map[String, Seq[Set[Int]]]
    ): Boolean = {
      if (pairs.isEmpty) {
        false
      } else {
          ...
          case Some(existingIndices) => {
            if (existingIndices.exists(x => x.intersect(indices).isEmpty)) {
              ...
              hasNonOverlappingPairs(
                pairs.tail,
                charIndices + (key -> (existingIndices :+ indices))
              )
            }
          }
          case None =>
    ...
    val isNice = trips.nonEmpty && hasNonOverlappingPairs(
      str.zipWithIndex.sliding(2).toSeq,
      Map.empty
    )
    isNice
  }
