...
object Day10 extends App{
  ...
  def lookAndSay(str : Vector[Char], timesRemaining : Int) : Vector[Char] = {
    ...
    def doTheThing(curChar : Char, charCount : Int, remainingString : Vector[Char], result : Vector[Char]) : Vector[Char] = {
      ...
      if(remainingString.isEmpty){
        ...
        if(next == curChar){
          ...
          val updatedResult = result.appendedAll(Seq(Character.forDigit(charCount, 10), curChar))
