...
object Day15 extends App{
  ...
  val best = allCombos.foldLeft(0L){ case (acc, s) =>
    val (capacity, durability, flavor, texture, calories) = s.foldLeft((0,0,0,0,0)){ case (acc, x)=>
      (acc._1 + x.capacity, acc._2 + x.durability, acc._3 + x.flavor, acc._4 + x.texture, acc._5 + x.calories)
    }

    val total = Math.max(capacity, 0) * Math.max(durability, 0) * Math.max(flavor, 0) * Math.max(texture, 0)
