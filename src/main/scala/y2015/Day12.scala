...
object Day12 extends App {
  ...
  val data =
    Source.fromResource("2015/12.data").getLines().mkString("").toVector
      ...
      nextChar match {
        case ',' => {
          getObjectValue(
            ...
            isRed,
            inArray
          ) // in between properties, keep going
        }
        case '[' =>
        ...
        case '}' =>
          if (isRed) (0, v.tail)
          else (acc, v.tail) // object is done..return 0 or acc
        case '"' => {
          // new property..scan until beginning of value

          val removedPropertyName =
            if (inArray) v else v.dropWhile(_ != ':').tail

          // determine type
          removedPropertyName.head match {
            case '[' => {
              val (value, remaining) =
                getObjectValue(removedPropertyName.tail, 0, false, true)
              getObjectValue(remaining, acc + value, isRed, inArray)
            }

            case '{' => {
              val (value, remaining) =
                getObjectValue(removedPropertyName.tail, 0, false, false)
              getObjectValue(remaining, acc + value, isRed, inArray)
            } // object beginning
            ...
            case _ => { // number beginning
              val i =
                removedPropertyName.indexWhere(x => !(x.isDigit || x == '-'))
              val (value, remaining) = removedPropertyName.splitAt(i)
              getObjectValue(
                remaining,
                acc + value.mkString("").toInt,
                isRed,
                inArray
              )
            }
