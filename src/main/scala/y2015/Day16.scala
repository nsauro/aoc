...
object Day16 extends App {
  ...
  val sueFragements = data.map {
    case SueFragmentData(
      ...
      SueFragment(
        id.toInt,
        Map(
          p1Name -> p1Value.toInt,
          p2Name -> p2Value.toInt,
          p3Name -> p3Value.toInt
        )
      )
    }
  }.toSeq
  ...
  val filteredOut = sueFragements.filterNot { x =>
    x.properties.exists(x =>
      knownProperties.contains(x._1) && knownProperties(x._1) != x._2
    ) ||
    x.properties.exists(x =>
      greaterThanProperties.contains(x._1) && greaterThanProperties(
        x._1
      ) >= x._2
    ) ||
    x.properties.exists(x =>
      lessThanProperties.contains(x._1) && lessThanProperties(x._1) <= x._2
    )
  }
