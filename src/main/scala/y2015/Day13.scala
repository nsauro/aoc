...
object Day13 extends App{
  ...
  val Happiness = raw"""(\w+) would (\w+) (\d+) happiness units by sitting next to (\w+).""".r

  val data = Source.fromResource("2015/13.data").getLines()

  val happinessMap = data.foldLeft(Map.empty[(String, String), Int]){ case (acc, item) =>
    item match{
  ...
  val allValues = people.permutations.map{ x =>
    val circle = x :+ x.head
    circle.sliding(2).map(x => happinessMap((x.head, x.last)) + happinessMap(x.last, x.head)).sum
  }
